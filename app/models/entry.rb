...
class Entry < ActiveRecord::Base
  ...
  attr_accessor :access_token, :link
  
  def send_caguei_comment
    data = { :access_token => self.access_token, :message => "cagou para isso.    Usando o aplicativo Caguei." }
  ...
  def creat_a_new_post_to_my_feed
    description = "Uma demonstração de indiferença para o post de #{self.user_name}. Enviada pelo aplicativo Caguei."
    picture = "http://lh3.ggpht.com/-Ol2ggZhgLL8/Sld22kHpItI/AAAAAAAAAGc/mY0eXbAlrAY/caguei.JPG"
    data = { :access_token => self.access_token, :message => "Caguei muito para o post de #{self.user_name}", :name => self.user_name + " via Caguei", :link => link, :description => description, :picture => picture }
